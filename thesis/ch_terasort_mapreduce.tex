...
最后将 TS-Cube 与 pipesort 结合，从这两个方面一同提高计算数据立方的效率。TS-Cube 可对大的group 进行划分，pipesort可减少中间数据，并且将多个 group 放在同一个 reduce函数中计算。由于 pipesort 的使用，无论是采样还是数据立方的计算，mapper 的输出不再是映射所有的 region，而仅需要映射这个pipeline 中的一个 region 即可。下面以 ABCD-\textgreater ABC-\textgreater AB-\textgreater A 这个pipeline，e1=[a0, b0, c0, d0, m0]这条记录为例，分别分析采样与数据立方计算时 mapper 的输出。

在TS-Cube 的第一轮 MapReduce中，mapper 输出的样本key应为[region\_id, group\_value, tuple\_id or measure\_value]。这里对应上述的pipeline 和记录，mapper 输出的key应为 [Region(A)\_id, a0, tuple\_id or measure\_value]。

在TS-Cube 的第二轮 MapReduce 中，mapper 输出的key应与第一轮是一致的，因此mapper 输出的key为[Region(A)\_id, a0, tuple\_id or measure\_value]，输出的value则为[b0, c0, d0] 这样的组合键。在上面的章节中提到，MapReduce的机制较为灵活，可重载 GroupComparator 函数，令只要有相同 region\_id 和 group\_value 的数据都放在同一个 reduce 函数中。因此在同一个 reducer 中，对于属性 A 具有相同值的数据都会放在同一个 reduce 函数里。
